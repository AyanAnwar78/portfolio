...
export default function Certifications() {
  ...
  return (
    <section className="py-24 px-6 md:px-12 lg:px-24 bg-transparent" id="certifications" ref={sectionRef}>
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 lg:gap-24">
        ...
        <div className="md:w-2/3 flex flex-col gap-16 bg-black p-8 md:p-16 rounded-[2rem] border border-white/5 relative z-10">
          {certifications.map((item, idx) => (
            <div key={idx} ref={(el) => (itemsRef.current[idx] = el)} className="flex flex-col gap-4 border-l border-white/10 pl-8 md:pl-12 relative group">
              {/* Timeline dot */}
              <div className="absolute left-[-5px] top-2 w-2.5 h-2.5 rounded-full bg-text-gray group-hover:bg-[#9cff00] transition-colors duration-300"></div>

              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h3 className="text-xl md:text-2xl font-bold text-text-light group-hover:text-[#9cff00] transition-colors duration-300">
                  {item.title}
                </h3>
                <span className="font-mono text-sm text-text-gray">{item.year}</span>
              </div>
              <h4 className="text-lg font-mono text-text-light/60 uppercase tracking-widest">{item.issuer}</h4>

              {/* Certificate Image */}
              {item.image && (
                <div className="mt-2 overflow-hidden rounded-xl border border-white/10 group-hover:border-[#9cff00]/30 transition-colors duration-300 max-w-md bg-black hidden lg:block">
                  <img
                    src={item.image}
